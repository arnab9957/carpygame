"""
This script updates the LEFT key handling in the SettingsMenu class to use the animation.
...
def update_left_key_handling():
    ...
    # Find the LEFT key handling code
    left_key_pattern = r'elif event\.key == pygame\.K_LEFT:.*?if result:\s+return result'
    match = re.search(left_key_pattern, content, re.DOTALL)
    
    if match:
        # Replace the LEFT key handling code
        new_content = content[:match.start()] + new_left_key_code + content[match.end():]
        
        # Write the modified content back to the file
        with open(file_path, 'w') as file:
            file.write(new_content)
        
        print("Successfully updated LEFT key handling in SettingsMenu class!")
    else:
        print("Could not find LEFT key handling code in SettingsMenu class.")

if __name__ == "__main__":
    update_left_key_handling()
