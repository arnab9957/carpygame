...
pygame.mixer.init()  # Initialize the mixer for sound effects"""
        ...
        game_class_match = re.search(game_class_pattern, updated_content)
        
        if not game_class_match:
            print("Error: Could not find Game class")
            return False
        
        # Find the __init__ method in the Game class
        init_pattern = r'class Game:.*?def __init__\(self\):.*?self\.reset_game\(\)'
        ...
        sound_loading = old_init.replace(
            "self.reset_game()",
            """# Load engine sounds
        self.load_sounds()
        ...
        # Add load_sounds method after reset_game method
        reset_game_pattern = r'def reset_game\(self\):.*?self\.set_mission\(\)'
        reset_game_match = re.search(reset_game_pattern, updated_content, re.DOTALL)
    ...
    def update_engine_sound(self):
        ...
        # Insert the load_sounds method after reset_game method
        updated_content = updated_content.replace(old_reset, old_reset + load_sounds_method)
        ...
        update_match = re.search(update_pattern, updated_content, re.DOTALL)
