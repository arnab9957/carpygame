...
class ParticleSystem:
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        for particle in self.particles:
            ...
            if (particle.x < -50 or particle.x > screen_rect.width + 50 or
                particle.y < -50 or particle.y > screen_rect.height + 50):
                continue
        ...
        if self.direction == "out":
            ...
            if self.to_surface:
                if direction == "left":
                    ...
                    self.screen.fill(self.transition_color)
                    self.screen.blit(self.to_surface, (0, 0), (0, 0, reveal_width, height))
                elif direction == "right":
                    reveal_width = int(width * self.progress)
                    self.screen.fill(self.transition_color)
                    self.screen.blit(self.to_surface, (width - reveal_width, 0), (width - reveal_width, 0, reveal_width, height))
                elif direction == "up":
                    reveal_height = int(height * self.progress)
                    self.screen.fill(self.transition_color)
                    self.screen.blit(self.to_surface, (0, 0), (0, 0, width, reveal_height))
                elif direction == "down":
                    reveal_height = int(height * self.progress)
                    self.screen.fill(self.transition_color)
                    self.screen.blit(self.to_surface, (0, height - reveal_height), (0, height - reveal_height, width, reveal_height))
        ...
        if self.direction == "out":
            ...
            for x in range(0, width, tile_size):
                for y in range(0, height, tile_size):
                    ...
                    tile_progress = max(0, min(1, (self.progress - random.random() * 0.3) / 0.7))
                    if tile_progress > 0:
...
class Game:
    ...
    def draw(self):
        try:
            ...
            pause_button_rect = pygame.Rect(SCREEN_WIDTH - 60, 100, 50, 40)  # Better position: more space from edges, larger button
            
            # Add subtle glow effect to make button more visible
            glow_rect = pause_button_rect.copy()
            glow_rect.inflate_ip(4, 4)
            pygame.draw.rect(self.screen, (DEEP_BLUE[0]//2, DEEP_BLUE[1]//2, DEEP_BLUE[2]//2), glow_rect, border_radius=7)
            ...
            pygame.draw.rect(self.screen, NEON_YELLOW, (left_bar_x, button_center_y - bar_height // 2, bar_width, bar_height))
            
            # Right pause bar
            right_bar_x = button_center_x + bar_spacing // 2
            pygame.draw.rect(self.screen, NEON_YELLOW, (right_bar_x, button_center_y - bar_height // 2, bar_width, bar_height))
            ...
            cli_text = cli_font.render("Amazon Q CLI", True, (255, 255, 255, 120))  # Slightly more transparent
            cli_rect = cli_text.get_rect(bottomright=(SCREEN_WIDTH - 10, SCREEN_HEIGHT - 10 - akd_rect.height - 2))  # 2px gap below AKD
    ...
    def update(self):
        try:
            ...
            current_time = time.time()
            dt = min(current_time - self.last_update_time, 1.0/30.0) if hasattr(self, 'last_update_time') else 1.0/60.0
            self.last_update_time = current_time
    ...
    def show_menu(self):
        ...
        try:
            ...
            screen_width = self.screen.get_width()
            screen_height = self.screen.get_height()
            background_image, has_background_image = load_background_image(screen_width, screen_height)
    ...
    def run(self):
        try:
            ...
            while running:
                ...
                current_time = time.time()
                dt = min(current_time - last_frame_time, 1.0/30.0)  # Cap dt to prevent large jumps
                ...
                if current_time - fps_update_time >= 1.0:
                    ...
                    if actual_fps < target_fps * 0.8:  # If FPS drops below 80% of target
